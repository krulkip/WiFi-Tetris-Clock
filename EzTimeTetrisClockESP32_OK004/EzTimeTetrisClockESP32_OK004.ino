...
char ssid[] = "Ziggo8773552";     // your network SSID (name)
char password[] = "xYf4bntkdjdu"; // your network key
...
void animationHandler()
{
  ...
  if (!finishedAnimating) {
    ...
    //display.fillScreen(tetris.tetrisBLACK);
    if (displayIntro) {
      finishedAnimating = tetris.drawText(1, 5+dropoffset/2+drop);
    } else {
      if (twelveHourFormat) {
        ...
        tetris1Done = tetris.drawNumbers(-6, 26+dropoffset, showColon);
        tetris2Done = tetris2.drawText(56, 25+dropoffset+drop);

        // Only draw the top letter once the bottom letter is finished.
        if (tetris2Done) {
          tetris3Done = tetris3.drawText(56, 15+dropoffset+drop);
        }

        finishedAnimating = tetris1Done && tetris2Done && tetris3Done;

      } else {
        finishedAnimating = tetris.drawNumbers(2, 26+dropoffset, showColon);
      }
    }
#ifdef double_buffer
    display.showBuffer();
...
void setup() {
  ...
  // "connecting"
  drawConnecting(5, 10+dropoffset/2+drop);
#ifdef double_buffer
  display.showBuffer();
#endif
...
#endif
  // "Powered By"
  drawIntro(5, 10+dropoffset/2+drop);
#ifdef double_buffer
  display.showBuffer();
#endif
  delay(2000);
...
void handleColonAfterAnimation() {
  ...
  int y = 26+dropoffset+drop*2 - (TETRIS_Y_DROP_DEFAULT * tetris.scale);
